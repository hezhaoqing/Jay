os.listdir(dirname)：列出dirname下的目录和文件                          ### 目录要加引号，Windows路径的\需要转义。
os.getcwd()：获得当前工作目录
os.curdir:返回当前目录（'.')
...
os.path.splitext():分离文件名与扩展名 （同样不会判断文件是否存在）
...
os.walk(dir)   ### 遍历dir下的所有子目录以及文件，返回多个3元素的元组。格式：（本目录，所有子目录，所有文件）（子目录1，所有孙目录，文件）......
                   多个子目录或者多个文件 则为列表，没有则为[]，
